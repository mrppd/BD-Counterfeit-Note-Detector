...
int image_registration_b(Mat template_image_gray, Mat input_image_gray, Mat &result_image,
...
JNIEXPORT jint JNICALL Java_net_pronaya_ndkdemo_nativeInterface_register_1image_1back
  (JNIEnv * env, jobject obj, jlong templateImgAddr, jlong testImgAddr){
    ...
    Mat result_image = input_image_gray.clone();
    float r_t;
    int x_t, y_t, match_points, max_r=180, min_r=50;

    for(int i=max_r; i>=min_r; i=i-20)
    {
            r_t = (float)i/100;
            x_t = (template_image_gray.cols*r_t);
            y_t = (template_image_gray.rows*r_t);

            //cout<<endl<<"Resize ratio: "<<r_t<<endl;

            Mat template_tmp;
            resize(template_image_gray, template_tmp, Size(x_t, y_t));

            match_points = image_registration_b(template_tmp, input_image_gray, result_image, 20);

            resize(result_image, result_image, Size(template_image_gray.cols, template_image_gray.rows));

            template_tmp.release();

            if(match_points>=20)
                break;
    }

    input_image_gray = result_image;
